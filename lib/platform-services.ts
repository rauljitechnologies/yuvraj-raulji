...
const MAGENTO_SERVICES: readonly PlatformService[] = [
  /* ── 01 Consulting ──────────────────────────────────────────── */
  {
    platform: 'magento',
    slug: 'consulting',
...
for (const [platform, services] of Object.entries(PLATFORM_SERVICES)) {
  ...
  /* The hub owns the entity term; a spoke may not claim it back. */
  seen.set(`${platform} consultant`, `${platform} (hub)`);
  for (const service of services) {
    const key = service.primaryKeyword.toLowerCase();
    const owner = seen.get(key);
    if (owner) {
      throw new Error(
        `Keyword collision: "${service.primaryKeyword}" is claimed by both ${owner} and ${platform}/${service.slug}`,
      );
    }
    seen.set(key, `${platform}/${service.slug}`);
  }
}
