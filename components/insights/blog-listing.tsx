  ...
  return (
    <div className="sec-bar mt-16 reveal">
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
        ...
        <span style={{ fontSize: '.68rem', fontWeight: 700, letterSpacing: '.22em', textTransform: 'uppercase', color: 'rgba(245, 243, 238,.55)' }}>
          {label}
        </span>
      </div>
  ...
  return (
    <article className="bc reveal group">
      <Link href={`/insights/${post.slug}`} className="block relative overflow-hidden flex-shrink-0" style={{ height: imgHeight }}>
        ...
        {showDate && (
          <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg,transparent 50%,rgba(5,5,5,.40) 100%)', pointerEvents: 'none' }} />
        )}
      </Link>

      <div className="flex flex-col flex-1 p-6 lg:p-7">
        <div className="flex items-center flex-wrap gap-[8px] mb-4">
          <span style={{ fontSize: '.62rem', fontWeight: 700, letterSpacing: '.15em', textTransform: 'uppercase', color: 'var(--accent-bright)' }}>{post.cat}</span>
          <span style={{ color: 'var(--text-faint)', fontSize: '.75rem', lineHeight: 1 }}>·</span>
          <span style={{ fontSize: '.60rem', fontWeight: 600, letterSpacing: '.12em', textTransform: 'uppercase', color: 'rgba(245, 243, 238,.55)' }}>{post.readTime}</span>
          {showDate && (
            <>
              <span style={{ color: 'var(--text-faint)', fontSize: '.75rem', lineHeight: 1 }}>·</span>
              <span style={{ fontSize: '.58rem', fontWeight: 600, letterSpacing: '.10em', textTransform: 'uppercase', color: 'rgba(245, 243, 238,.55)' }}>{post.date}</span>
            </>
          )}
        </div>

        <h2
          className="font-display uppercase leading-[1.06] text-[#f5f3ee] mb-3 flex-shrink-0 transition-colors duration-200 group-hover:text-rv"
          style={{ fontSize: titleSize, letterSpacing: '.02em' }}
        >
          {post.title}
        </h2>

        <p className="leading-[1.74] line-clamp-2 flex-1 mb-5" style={{ fontSize: excerptSize, color: showDate ? 'rgba(245, 243, 238,.55)' : 'rgba(245, 243, 238,.55)' }}>
          {post.excerpt}
        </p>
        ...
        <div
          style={{
            height: 1,
            background: showDate
              ? 'linear-gradient(90deg,rgba(215, 25, 32,.18),rgba(255,255,255,.04),transparent)'
              : 'linear-gradient(90deg,rgba(215, 25, 32,.18),transparent)',
        ...
        <Link
          ...
          style={{ fontSize: showDate ? '.70rem' : '.68rem', fontWeight: 700, letterSpacing: '.13em', textTransform: 'uppercase', color: 'var(--accent-bright)' }}
        >
...
export function BlogListing({ posts }: { posts: ListPost[] }) {
  ...
  return (
    <>
      {/* ══ FILTER BAR ══ */}
      <div
        className="sticky top-[74px] z-[100] bg-[rgba(5,5,5,.96)] backdrop-blur-2xl"
        ...
        <div className="max-w-shell mx-auto px-6 md:px-10">
          <div className="flex items-center justify-between gap-4 py-4">
            <div className="flex items-center gap-3 overflow-x-auto scrollbar-hide flex-1 min-w-0">
              <span className="flex-shrink-0 text-[.58rem] font-bold tracking-[.24em] uppercase text-[rgba(245,243,238,.55)] pr-2 border-r border-[rgba(255,255,255,.08)] mr-1 hidden sm:block">
                Filter
              </span>
              {FILTERS.map((f) => (
                <button
                  ...
                  className={`flex-shrink-0 h-[34px] px-[18px] rounded-full text-[.63rem] font-bold tracking-[.12em] uppercase whitespace-nowrap transition-all duration-200 touch-manipulation border ${
                    activeFilter === f
                      ? 'bg-accent text-white border-accent shadow-[0_4px_18px_rgba(238,42,52,.38)]'
                      : 'text-[rgba(245,243,238,.55)] border-[rgba(255,255,255,.08)] hover:text-[#f5f3ee] hover:border-[rgba(215,25,32,.24)] hover:bg-[rgba(215,25,32,.06)]'
                  }`}
                >
                  {FILTER_LABELS[f]}
                </button>
            ...
            <div className="flex-shrink-0 flex items-center gap-2">
              ...
              <span className="text-[.62rem] font-bold tracking-[.18em] uppercase text-[rgba(245,243,238,.55)]">{filtered.length} Articles</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="max-w-shell mx-auto px-6 md:px-10">
          {slides.length > 0 && (
            <div className="mb-20 reveal">
              <div className="sec-bar">
                ...
                <span className="sec-line" />
                <span className="font-display" style={{ fontSize: '1.20rem', letterSpacing: '.06em', color: 'rgba(245, 243, 238,.55)' }}>
                  <span style={{ color: 'var(--accent-bright)' }}>{pad(slCur + 1)}</span>
                      ...
                      >
                        ...
                        <div
                          style={{
                            position: 'absolute',
                            inset: 0,
                            background: 'linear-gradient(0deg,rgba(5,5,5,.98) 0%,rgba(5,5,5,.70) 38%,rgba(5,5,5,.12) 70%,rgba(5,5,5,.04) 100%)',
                            pointerEvents: 'none',
                          }}
                        />
                        <div
                          className="group-hover:opacity-100"
                          style={{ position: 'absolute', inset: 0, borderRadius: 'inherit', boxShadow: 'inset 0 0 0 1px rgba(215, 25, 32,.34)', opacity: 0, transition: 'opacity .28s', pointerEvents: 'none' }}
                        />
                        ...
                        <div className="absolute top-5 left-5 sm:top-7 sm:left-8 flex items-center gap-[12px]">
                          <span
                            ...
                            style={{ background: 'var(--accent)', fontSize: '.60rem', fontWeight: 700, letterSpacing: '.16em', textTransform: 'uppercase', color: '#fff', boxShadow: '0 6px 20px rgba(215, 25, 32,.45)' }}
                          >
                            {post.cat}
                          </span>
                          <span style={{ fontSize: '.60rem', fontWeight: 600, letterSpacing: '.14em', textTransform: 'uppercase', color: 'rgba(245, 243, 238,.55)' }}>
                            {post.readTime}
                          </span>
                        </div>

                        <div className="sl-body absolute bottom-0 left-0 right-0">
                          <div style={{ fontSize: '.62rem', fontWeight: 700, letterSpacing: '.20em', textTransform: 'uppercase', color: 'rgba(245, 243, 238,.55)', marginBottom: 10 }}>
                            {post.date}
                          </div>
                          <h2
                            className="font-display uppercase text-[#f5f3ee] transition-colors duration-200 group-hover:text-rv"
                            style={{ fontSize: 'clamp(1.65rem,3.8vw,3.2rem)', lineHeight: 0.94, letterSpacing: '.02em', marginBottom: 12 }}
                          >
                            {post.title}
                          </h2>
                          <p className="hidden sm:block leading-[1.74] line-clamp-2" style={{ fontSize: '.90rem', color: 'rgba(245, 243, 238,.55)', maxWidth: 580, marginBottom: 20 }}>
                            {post.excerpt}
                          </p>
                          <div style={{ height: 1, background: 'linear-gradient(90deg,rgba(215, 25, 32,.24),rgba(255,255,255,.06),transparent)', marginBottom: 16 }} />
                          <div className="flex items-center justify-between flex-wrap gap-3">
                            <div className="hidden sm:flex items-center gap-[10px]">
                              <div
                                className="font-display"
                                style={{ width: 32, height: 32, borderRadius: '50%', border: '1px solid rgba(215, 25, 32,.30)', background: 'rgba(215, 25, 32,.10)', display: 'grid', placeItems: 'center', fontSize: '.72rem', color: 'var(--accent-bright)', flexShrink: 0 }}
                              >
                                YR
                              </div>
                              <div>
                                <div style={{ fontSize: '.72rem', fontWeight: 600, color: 'rgba(245, 243, 238,.78)' }}>Yuvraj Raulji</div>
                                <div style={{ fontSize: '.58rem', fontWeight: 500, letterSpacing: '.06em', color: 'rgba(245, 243, 238,.55)' }}>
                            ...
                            <span
                              ...
                              style={{ fontSize: '.70rem', fontWeight: 700, letterSpacing: '.16em', textTransform: 'uppercase', color: 'var(--accent-bright)' }}
                            >
                              Read Article <span className="transition-transform duration-300 group-hover:translate-x-1">→</span>
                            </span>
                          </div>
                  ...
                  >
                    <span
                      ...
                      className={`block rounded-full transition-all duration-400 ${
                        slCur === i ? 'bg-rv' : 'bg-[rgba(255,255,255,.20)] hover:bg-[rgba(215,25,32,.40)]'
                      }`}
                      style={slCur === i ? { width: 38, height: 11, boxShadow: '0 2px 16px rgba(238, 42, 52,.45)' } : { width: 11, height: 11 }}
                    />
                  </button>
                ))}
              </div>
          ...
          {filtered.length === 0 && (
            <div className="text-center py-28">
              <div className="inline-flex items-center justify-center w-16 h-16 rounded-full border border-[rgba(215,25,32,.18)] mb-6" style={{ background: 'rgba(215, 25, 32,.05)' }}>
                <span className="font-display text-rv" style={{ fontSize: '1.5rem' }}>
                  —
                </span>
              </div>
              <p className="text-[rgba(245,243,238,.55)] font-semibold tracking-[.14em] uppercase" style={{ fontSize: '.76rem' }}>
