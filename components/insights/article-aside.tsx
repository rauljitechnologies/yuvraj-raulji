...
const shareBtnCls =
  'w-10 h-10 grid place-items-center rounded-full cursor-pointer bg-transparent transition-all duration-300 hover:-translate-y-[2px] hover:!border-rv hover:!text-rv hover:shadow-[0_8px_28px_rgba(215,25,32,.28)]';
const shareBtnStyle: React.CSSProperties = { border: '1px solid rgba(255,255,255,.12)', color: 'rgba(245, 243, 238,.55)', fontSize: '.74rem', fontWeight: 700 };

function PanelHeading({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex items-center gap-3 mb-5">
      {spine}
      <p className="text-[.60rem] font-bold tracking-[.26em] uppercase text-[rgba(245,243,238,.55)]">{children}</p>
    </div>
  );
}
  ...
  return (
    <div className={mobile ? `lg:hidden p-6 rounded-xl border border-[rgba(255,255,255,.07)] bg-bg2 mb-10` : `hidden lg:block ${panelCls}`}>
      <div className={`flex items-center gap-3 ${mobile ? 'mb-4' : 'mb-5'}`}>
        {spine}
        <p className="text-[.60rem] font-bold tracking-[.26em] uppercase text-[rgba(245,243,238,.55)]">On This Page</p>
      </div>
          ...
          return (
            <a
              ...
              className={`flex items-start gap-[10px] py-1.5 text-[.79rem] leading-[1.5] transition-colors duration-200 ${
                mobile ? 'text-[rgba(245,243,238,.55)] hover:text-rv' : on ? 'text-rv' : 'text-[rgba(245,243,238,.55)] hover:text-[#f5f3ee]'
              }`}
            >
              <span
                className={`font-display pt-[2px] flex-shrink-0 ${mobile ? 'text-[#ee2a34]' : on ? 'text-rv' : 'text-[#ee2a34]'}`}
...
export function ArticleAside({ toc, tags, title }: { toc: Heading[]; tags: string[]; title: string }) {
  ...
  return (
    <aside className="lg:sticky lg:top-[104px] flex flex-col gap-6">
      ...
      <div className={panelCls}>
        ...
        <div className="flex items-center gap-4 mb-4">
          <div
            ...
            style={{ border: '1px solid rgba(215, 25, 32,.35)', background: 'rgba(215, 25, 32,.08)', boxShadow: '0 0 28px rgba(215, 25, 32,.14)' }}
          >
            YR
          </div>
          <div>
        ...
        <div className="flex flex-col gap-[10px]">
          ...
          <a
            ...
            target="_blank"
            rel="noopener"
            className="flex items-center justify-center gap-2 h-11 rounded border border-[rgba(255,255,255,.12)] text-[.70rem] font-semibold tracking-[.12em] uppercase text-[rgba(245,243,238,.60)] transition-all hover:border-[rgba(215,25,32,.36)] hover:text-rv touch-manipulation"
          >
            Connect on LinkedIn
          </a>
        </div>
      ...
      <div className={panelCls}>
        ...
        <div className="flex items-center gap-[10px]">
          ...
          <button
            ...
            style={
              copied
                ? { border: '1px solid var(--accent-bright)', color: 'var(--accent-bright)', fontSize: '.80rem', fontWeight: 700 }
      ...
      {/* Contact CTA */}
      <div className="relative p-7 rounded-xl overflow-hidden" style={{ border: '1px solid rgba(215, 25, 32,.26)', background: 'linear-gradient(160deg,rgba(215, 25, 32,.10),rgba(215, 25, 32,.03))' }}>
        <div className="absolute top-0 left-0 right-0 h-[2px]" style={{ background: 'linear-gradient(90deg,transparent,rgba(238, 42, 52,.65),transparent)' }} />
        ...
        <p className="text-[.78rem] text-[rgba(245,243,238,.55)] leading-[1.65] mb-5">Magento 2 · Shopify · SEO/CRO · AWS · WordPress · AI Automation</p>
        <a
          href={`mailto:${CONTACT.email}?subject=Consultation+Request`}
          className="flex items-center justify-center gap-2 h-11 rounded bg-red text-white text-[.70rem] font-bold tracking-[.12em] uppercase transition-all hover:bg-rv hover:shadow-[0_12px_36px_rgba(215,25,32,.34)] hover:-translate-y-px active:scale-[.97] touch-manipulation"
        >
          Book Now →
        </a>
      </div>
          ...
          <div className="flex flex-wrap gap-2">
            {tags.map((tag) => (
              <span
                ...
                style={{ border: '1px solid rgba(215, 25, 32,.20)', background: 'rgba(215, 25, 32,.05)', color: 'rgba(245, 243, 238,.55)' }}
              >
                {tag}
              </span>
            ))}
...
export function ReadingProgress() {
  ...
  return (
    <div className="fixed top-[74px] left-0 right-0 z-[810] h-[2px] bg-transparent" aria-hidden="true">
      <div style={{ height: '100%', width: `${pct}%`, background: 'linear-gradient(90deg,var(--accent),var(--accent-bright))', transition: 'width .1s linear' }} />
    </div>
  );
}
