    ...
    >
      <div className="max-w-shell mx-auto px-10 w-full">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_minmax(280px,.55fr)] gap-10 items-end mb-[clamp(32px,5vh,60px)] reveal">
          ...
          <div>
            <p className="text-[rgba(244,244,244,.60)] mb-5" style={{ fontSize: 'clamp(.92rem,1.2vw,1.06rem)', lineHeight: 1.74 }}>
              Editorial perspectives on the forces reshaping commerce, enterprise technology, and AI-powered operations.
            </p>
            <Link
              href="/blog"
              className="btn-arr inline-flex items-center gap-2 h-[42px] px-5 rounded border border-[rgba(200,16,46,.36)] text-rv text-[.70rem] font-bold tracking-[.12em] uppercase no-underline transition-all hover:bg-red hover:border-red hover:text-white hover:-translate-y-px touch-manipulation"
            >
              View All Insights
            </Link>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 border border-[rgba(255,255,255,.08)] rounded-xl overflow-hidden gap-px bg-[rgba(255,255,255,.08)] reveal">
          {INSIGHT_CARDS.map((p, i) => (
            <Link
              key={p.t}
              href="/blog"
              className="group relative min-h-[225px] p-6 bg-bg2 grid content-between gap-4 no-underline text-inherit overflow-hidden transition-all duration-300 hover:bg-[rgba(200,16,46,.06)] hover:-translate-y-1 touch-manipulation"
            >
              <span
                className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-[#e8192c] via-[#c8102e] to-transparent scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left"
                aria-hidden="true"
              />
              ...
              <span className="block">
                <h3
                  className="font-bebas uppercase tracking-[.03em] leading-[1.12] transition-colors duration-300 group-hover:text-white"
                ...
                <span className="flex items-center gap-2 mt-3 text-[.62rem] font-bold tracking-[.16em] uppercase">
                  <span className="text-[rgba(232,25,44,.80)]">{p.c}</span>
                  <span className="w-[3px] h-[3px] rounded-full bg-[rgba(244,244,244,.25)]" aria-hidden="true" />
                  <span className="text-[rgba(244,244,244,.40)] transition-colors duration-300 group-hover:text-[rgba(244,244,244,.65)]">{p.m}</span>
                </span>
              </span>
            </Link>
          ))}
